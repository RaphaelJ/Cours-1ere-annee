 ...
 *
 * Les expressions acceptent des modificateurs de
 * répétition qui se placent directement après les
 * expressions :
 *	?: 0 ou une fois ;
 *	*: 0 fois ou plus ;
 *	+: une fois ou plus ;
 ...
 *
 * TODO: Accepter l'opérateur OR (« | ») ainsi que les
 * sous-expressions (« (expression) »).
 */
...
typedef struct _TOKEN {
         ...
         * si un caractère remplis les conditions du
         * TOKEN.
	 *
	 * @TOKEN_TYPE: token à vérifier ;
	 * @char: caractère à vérifier avec le token.
	 *
	 * Sortie: true si dans le caractère respecte
	 * 	les conditions du token. */
        bool (*check)(const TOKEN_TYPE, const char);
